//! Implements the `BlockHooks` trait for the `ValueSetterZk` module.
//!
...
use crate::ValueSetterZk;

impl<S: Spec> BlockHooks for ValueSetterZk<S> {
    type Spec = S;
...
#[cfg(feature = "native")]
impl<S: Spec> FinalizeHook for ValueSetterZk<S> {
    type Spec = S;
    fn finalize_hook(
        &mut self,
